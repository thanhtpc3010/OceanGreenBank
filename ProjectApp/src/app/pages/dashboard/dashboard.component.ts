...
export class DashboardComponent implements OnInit {
  ...
  protected readonly autoEarnActive = signal(false);
  ...
  protected readonly maxFlow = Math.max(
    ...this.cashflow.map((c) => Math.max(c.income, c.expense)),
  );
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (firstActive) {
      ...
      void this.loadRecent(firstActive.id);
    }
